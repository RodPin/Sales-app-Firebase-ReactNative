...
export default class EncomendasAndamento extends Component {
  ...
  sent(numero, nCliente, lucro, price, prod, statusPagamento) {
    var ref = firebase
      .database()
      .ref()
      .child("Encomendas/" + numero);
    ref.once("value", function(snapshot) {
      if (snapshot.val().statusPagamento === "sim") {
        firebase
          .database()
          .ref()
          .child("EncomendasFinalizadas")
  ...
  renderRow(data) {
    return (
      <View
        style={{
          backgroundColor:
            data.data.statusPagamento === "nao" ? "#ff4d4d" : "#b3ff99",
          height: 45,
          flexDirection: "row",
      ...
      >
        <TouchableHighlight
          onPress={() => this.changeStatus(data.data.statusPagamento, data.key)}
        >
